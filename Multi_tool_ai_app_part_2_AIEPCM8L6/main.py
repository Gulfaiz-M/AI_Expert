...
# ---- Part 2 code: safe ai image generator logic ----
def run_safe_ai_image_generator():
    FORBIDDEN = ["violence","weapon","gun","blood","nude","porn","drugs","hate","racism","sex",
...
# add this model if the existing model is not working below --> "black-forest-labs/FLUX.1-schnell"
    ...
    def generate_image(prompt: str):
        if BAD.search(prompt): return None, "⚠️ Unsafe prompt."
        try: return img_client.text_to_image(prompt=prompt, model=IMG_MODEL), None
        except Exception as e: return None, f"Error during image generation: {e}"
    ...
    if ok:
        if not p.strip(): st.warning("⚠️ Enter a description.")
        else:
